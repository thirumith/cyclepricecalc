...
@Repository
public interface ChainSilverRepository extends JpaRepository<ChainSilverEntity, Long> {
	@Query("SELECT t FROM ChainSilverEntity t WHERE :date between t.fromDate and t.toDate")
	ChainSilverEntity searchPriceByBetweenDate(@Param("date")Date date);
}
