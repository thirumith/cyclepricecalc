...
@Repository
public interface WheeltubeRepository extends JpaRepository<WheelTubeEntity, Long> {
	@Query("SELECT t FROM WheelTubeEntity t WHERE :date between t.fromDate and t.toDate")
	WheelTubeEntity searchPriceByBetweenDate(@Param("date")Date date);
}
